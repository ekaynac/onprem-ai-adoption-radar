...
desk_app = typer.Typer(help="Analyst desk: weekly brief + public calls ledger.", no_args_is_help=True)
app.add_typer(desk_app, name="desk")
news_app = typer.Typer(help="Newsroom: feed sweep + LLM change classification.", no_args_is_help=True)
app.add_typer(news_app, name="news")
alerts_app = typer.Typer(help="Stack-profile alerts: compute + webhook delivery.", no_args_is_help=True)
...
def _news_config_path(root: Path) -> Path:
    ...
    if path.exists():
        return path
    return Path(__file__).resolve().parents[2] / "config" / "news-sources.yaml"
